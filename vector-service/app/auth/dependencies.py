"""
FastAPI dependencies for auth — mirrors quotation-service/src/auth/middleware.ts:
`require_auth` (human JWT login), `require_role` (JWT + role check), and
`require_api_key_or_auth` (accepts EITHER a service API key `vsk_...`, for the
main Node app calling POST /api/instruments/search, OR a human JWT).
