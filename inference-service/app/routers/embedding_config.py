"""Admin config for the embedding vLLM sidecar (baseUrl, model)."""
...
def get_embedding_config():
    return {"success": True, "config": db.get_embedding_settings()}


@router.put("/embedding")
def update_embedding_config(payload: UpdateEmbeddingConfigRequest, current: dict = Depends(require_role("admin"))):
    updated = db.update_embedding_settings(payload.model, payload.baseUrl, current["userId"])
